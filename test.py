...
# test = AF(r'C:\Users\Nate\Documents\UbuntuSharedFiles\fibergeneration\demos\cylinder.x3d', np.array([0.0, 0.0, 1.0]), np.array([np.cos(np.deg2rad(angle)), np.sin(np.deg2rad(angle)), 0.0]), np.array([0, 0, 1]), fiberint=0.05, plotting=True)
...
test = AF(r'C:\Users\Nate\Documents\UbuntuSharedFiles\fibergeneration\demos\smallsaddle.x3d', np.array([0.0, 1.0, 0.0]), np.array([-np.cos(np.deg2rad(angle)), 0, -np.sin(np.deg2rad(angle))]), np.array([0, 1, 0]), plotting=True, offset=0.01, fiberint=0.01, save=True)
