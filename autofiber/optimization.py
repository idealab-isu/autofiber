...
def computeglobalstrain_grad_test(normalized_2d, fiberpoints, vertexids, stiffness_tensor):
    import pdb
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            duvw_duij = np.zeros(rel_uvw.shape)
            duvw_duij[:, i, j] = 1
    ...
    dE_dstrain = 0.5 * areas * np.matmul(strain_vector + np.matmul(strain_vector, np.eye(strain_vector.shape[1], strain_vector.shape[1])), stiffness_tensor)

    dF_duvw = np.zeros((F.shape[0], 9, 2, 2))
    for i in range(0, 3):
        for j in range(0, 3):
            duvw_duij = np.zeros(rel_uvw.shape)
            duvw_duij[:, i, j] = 1
            dF_duvw[:, i*3+j, :, :] = np.matmul(rel_3d, -1*np.matmul(np.matmul(np.linalg.inv(rel_uvw), duvw_duij), np.linalg.inv(rel_uvw)))[:, :2, :2]

    dstrainvector_du = np.zeros((strain_vector.shape[0], strain_vector.shape[1], 6))
    for i in range(0, 6):
        dstrain_du = 0.5 * (np.matmul(dF_duvw[:, i, :, :], F.transpose(0, 2, 1)) + np.matmul(F, dF_duvw[:, i, :, :].transpose(0, 2, 1)))
        dstrainvector_du[:, :, i] = np.divide(np.array([[dstrain_du[:, 0, 0]], [dstrain_du[:, 1, 1]], [dstrain_du[:, 0, 1]]]).transpose((2, 0, 1)), m).squeeze()[np.newaxis]

    dE_du = np.matmul(dE_dstrain[:, np.newaxis], dstrainvector_du).reshape(dstrainvector_du.shape[0], 3, 2)
